...
function Container(modules, parent) {
  ...
  function invokeForAll(method) {
    ...
    function next(err) {
      ...
      var result;
      var instance = get(names[i++]);
      if (instance && typeof instance === 'object' && typeof instance[method] === 'function') {
        result = invoke(instance[method], instance);
      }
    ...
    // return Promise.each(Object.keys(providers), function (name) {
    //   var instance = get(name);
    //   if (instance && typeof instance === 'object' && typeof instance[method] === 'function') {
    //     return invoke(instance[method], instance);
    //   }
    // });
  }
  ...
  function start() {
    if (started) return;
    started = true;
    return invokeForAll('start');
  }

  function stop() {
    if (!started) return;
    started = false;
    return invokeForAll('stop');
  }
